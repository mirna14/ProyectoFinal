...
const addPost = async (req, res) => {
  ...
  try {
    ...
    req.flash('successMessage', 'Usuario registrado con exito');
    return res.redirect('/project');
  } catch (error) {
    log.error(
      'ln 53 project.controller: Error al registrar usuario en la base de datos'
    );
...
const edit = async (req, res) => {
  ...
  try {
    ...
    if (project === null) {
      log.info(`No se encontro al usuario con el id: ${id}`);
      return res
        .status(404)
        .json({ fail: `No se encontro al usuario con el id: ${id}` });
    }
...
const editPut = async (req, res) => {
  ...
  // se le informa al cliente
  if (validationError) {
    log.info(`Error de validación del usuario con id: ${id}`);
    // Se desestructuran los datos de validación
    const { value: project } = validationError;
    // Se extraen los campos que fallaron en la validación
    const errorModel = validationError.inner.reduce((prev, curr) => {
  ...
  project.apellidos = newProject.apellidos;
  project.correo = newProject.correo;
  project.curp = newProject.curp;
  project.rfc = newProject.rfc;
  project.contrasena = newProject.contrasena;
  try {
